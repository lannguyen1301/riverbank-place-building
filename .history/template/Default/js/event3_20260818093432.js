...
input.addEventListener("input", function () {
    ...
    const results = buildings.filter((b) =>
        b.toanha.toLowerCase().includes(keyword),
    );
    if (keyword === "") {
    ...
    const resultsNoDiacritics = buildings.filter((b) =>
        removeDiacritics(b.toanha.toLowerCase()).includes(
            removeDiacritics(keyword),
        ),
    );
    suggestion.innerHTML = resultsNoDiacritics
        .slice(0, 1000)
        .map(
            (item) =>
                `<li class="cus_flex_end" title="Click xem tòa nhà"> <a class="" href="` +
                item.uri +
                `.html` +
                `" target="_blank">` +
                item.toanha +
